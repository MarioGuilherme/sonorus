...
public class ChatController : APIControllerBase {
    ...
    public async Task<ActionResult<RestResponse<List<ChatDTO>>>> GetAllChats() {
        ...
        try {
            response.Data = await this._chatService.GetAllChatsByUserIdAsync((int)this.CurrentUser!.UserId!);
    ...
    public async Task<ActionResult<RestResponse<List<MessageDTO>>>> GetAllMessages(Guid chatId) {
        RestResponse<List<MessageDTO>> response = new();
        try {
            response.Data = await this._chatService.GetAllMessagesByChatIdAsync(chatId, (int) this.CurrentUser!.UserId!);
            return this.Ok(response);
        } catch (SonorusChatAPIException exception) {
            response.Message = exception.Message;
            response.Errors = exception.Errors;
